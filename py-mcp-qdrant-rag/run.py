...
def run_mcp_server(config: Dict[str, Any]):
    ...
    try:
        ...
        # Change stdio_server to sse_server and provide network details
        mcp_server = FastMCP(name="pyragdoc")
        mcp_server.run(transport="http", host="0.0.0.0", port=8082, path="/mcp")

        logger.info("PyRAGDoc MCP Server is ready http://127.0.0.1:8082")
